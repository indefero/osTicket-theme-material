...
<select name="<?php echo $TZ_NAME; ?>" id="timezone-dropdown"
        data-placeholder="<?php echo $TZ_PLACEHOLDER; ?>" class="selectpicker show-tick">
        ...
        <?php }
            foreach (DateTimeZone::listIdentifiers() as $zone) { ?>
                <option value="<?php echo $zone; ?>" <?php
                    ...
                    ?>><?php echo str_replace('/',' / ',$zone); ?>
                </option>
            <?php } ?>
</select>
<button type="button" class="action-button waves-effect" onclick="javascript:
...
var recheck = setInterval(function() {
    if (window.jstz !== undefined) {
        ...
        $('#timezone-dropdown').selectpicker('val', zone.name());
    }
}, 100);
return false;" style="vertical-align:middle"><i class="icon-map-marker"></i> <?php echo __('Auto Detectar'); ?></button>
